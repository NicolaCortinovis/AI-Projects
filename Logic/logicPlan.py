...
def sentence1() -> Expr:
    ...
    first_phrase = A|B
    second_phrase = (~A) % ((~B)|C)
    third_phrase = disjoin([(~A),(~B),(C)])
    
    return(conjoin([first_phrase,second_phrase,third_phrase]))
    "*** END YOUR CODE HERE ***"
...
def sentence2() -> Expr:
    ...
    return(conjoin([first_phrase,second_phrase,third_phrase,fourth_phrase]))
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    first_phrase = Alive1 % ((Alive0 & (~Killed0)) | (~(Alive0) & Born0))
    second_phrase = ~(Alive0 & Born0)
    third_phrase = Born0
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # entail: A \models B if and only if A \Rightarrow B is a tautology which we can repurpose as 
    # A \models B if and only if \neg(A \Rightarrow B) is unsatisfiable
    new_expr = ~(premise >> conclusion)
    return(not findModel(new_expr))
    "*** END YOUR CODE HERE ***"
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # A disjoin of literals is just a CNF with a single phrase
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    # For each possible pairing of 2 literals A, B only one must be True, this is verified if 
    # (\neg A \vee \neg B) is True. Note that the 2 literals must be different for this to work
    
    PairsList = []
    
    for literal1,literal2 in itertools.combinations(literals,2):
        if(literal1 != literal2):
            PairsList.append(disjoin(~literal1, ~literal2))

    return conjoin(PairsList)
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    # To check that there exists exactly one literal that is True we can simply check that 
    # both atLeastOne and atMostOne are verified
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x,y in non_outer_wall_coords:
        coordinates.append(PropSymbolExpr(pacman_str, x, y, time = t))
        
    pacphysics_sentences.append(exactlyOne(coordinates))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # PacphysicsAxioms for t = 0 and t = 1
    KB.append(pacphysicsAxioms(t = 0, all_coords = all_coords, non_outer_wall_coords = non_outer_wall_coords, walls_grid= walls_grid,
                               sensorModel= None, successorAxioms=allLegalSuccessorAxioms))
    KB.append(pacphysicsAxioms(t = 1, all_coords = all_coords, non_outer_wall_coords = non_outer_wall_coords, walls_grid= walls_grid,
                               sensorModel= None, successorAxioms=allLegalSuccessorAxioms))
    
    # Pacman current location
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time = 0))
    
    # Pacman taking action0
    KB.append(PropSymbolExpr(action0, time = 0))
    
    # Pacman taking action1
    KB.append(PropSymbolExpr(action1, time = 1))
    
    # Pacman at (x1,y1) at time = 1 given starting position x0,y0 action 0 and action 1
    model1 = findModel(conjoin([conjoin(KB), PropSymbolExpr(pacman_str, x1, y1, time = 1)]))
    
    # Pacman not at (x1,y1) at time = 1 given starting position x0,y0 action 0 and action 1
...
def positionLogicPlan(problem) -> List:
    ...
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time = 0)) # Initial KB
    
    for t in range(50):
        print("Timestep: {}".format(t))     # Timestep print
        
        # From all the possbile legal (non wall) locations at time t
        legal_locations = []
        for (x,y) in non_wall_coords:
            legal_locations.append(PropSymbolExpr(pacman_str, x, y, time = t))
        
        # Pacman can be in exactly one square
        KB.append(exactlyOne(legal_locations))
        
        # If there is an assignment of actions that satisfies pacman is in goal state and the KB both at time t, then
        # extract that sequence and return it
        model = findModel(conjoin([PropSymbolExpr(pacman_str, xg, yg, time = t), conjoin(KB)]))
        if(model):
            return extractActionSequence(model, actions)
        
        # Of all the legal actions pacman can take at time t
        legal_actions = []
        for action in actions:
            legal_actions.append(PropSymbolExpr(action, time = t))
        
        # Add to the KB only one
        KB.append(exactlyOne(legal_actions))
        
        # Add to KB all possible transition models sentences
        for (x,y) in non_wall_coords:
...
def foodLogicPlan(problem) -> List:
    ...
    # What we initially know about pacman and the food
    
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time = 0)) # Initial KB
    
    for (x,y) in food:
            KB.append(PropSymbolExpr(food_str,x,y, time = 0)) # Food position
    ...
    for t in range(50):
        ...
        food_pos = []
        for (x,y) in food:
            food_pos.append(PropSymbolExpr(food_str,x,y, time = t))
        ...
        # New goal assertion: only if there is no more food on the map we have won
        goal = ~disjoin(food_pos)
        
        # If there exists a sequence of actions that satisfies the CNF of KB and goal...
        model = findModel(conjoin([conjoin(KB), goal]))
        if(model):
            return extractActionSequence(model, actions) # return it
        
        # Add to KB all possible transition models sentences
        for (x,y) in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x,y, time = t+1, walls_grid = walls_grid))
            
        # Add to KB food successor axiom:
        # Food in (x,y) at t+1 is NOT present if and only if pacman was there at time t or there wasn't
        # any food at the previous step t
        
        for (x,y) in non_wall_coords:
            pacman_position = PropSymbolExpr(pacman_str, x,y, time = t)
            food_curr_step = PropSymbolExpr(food_str, x, y, time = t)
            food_next_step = PropSymbolExpr(food_str, x, y, time = t+1)
            
            # \neg food_next_step \Leftrightarrow (pacman_position \vee \neg food_curr_step)
            KB.append(~food_next_step % (pacman_position | ~food_curr_step))
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        # Add pacphysics at time t
        KB.append(pacphysicsAxioms(t = t, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, walls_grid=walls_grid,
                               sensorModel=sensorAxioms, successorAxioms=allLegalSuccessorAxioms))
        
        # Add possible actions
        KB.append(PropSymbolExpr(agent.actions[t], time = t))
        
        # Add what the percepts see
        ...
        for (x,y) in non_outer_wall_coords:
            ...
            if findModel(conjoin([conjoin(KB), Pacman_position])): # If a satisfiable assignment exists given the current KB
                possible_locations.append((x,y))   
            if entails(conjoin(KB),Pacman_position): # Location where pacman can be proved  to be in at time t
                KB.append(Pacman_position)
            elif (entails(conjoin(KB),~Pacman_position)): # Location where pacman can't be proved  to be in at time t
                KB.append(~Pacman_position)
...
def mapping(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    initial_location = PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time = 0)
    KB.append(initial_location)
    
    # 1 -> wall, 0 -> not wall, -1 -> ambiguos
    # known_map is our wall 
    ...
    for t in range(agent.num_timesteps):
        ...
        KB.append(pacphysicsAxioms(t = t, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, walls_grid=known_map,
                               sensorModel=sensorAxioms, successorAxioms=allLegalSuccessorAxioms))
        # Add possible actions
        KB.append(PropSymbolExpr(agent.actions[t], time = t))
        ...
        for (x,y) in non_outer_wall_coords:
            ...
            if entails(conjoin(KB),wall_position): # Location where walls can be proved to be in
                KB.append(wall_position)
                known_map[x][y] = 1 # Update the map with a 1 in (x,y)
                
            elif (entails(conjoin(KB),~wall_position)): # Location where walls can be proved not to be in
                KB.append(~wall_position)
                known_map[x][y] = 0 # Update the map with a 0 in (x,y)
